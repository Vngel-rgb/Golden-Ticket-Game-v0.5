# Golden Ticket Game — mechanics playtest v0.6

This branch preserves the working v0.5 lobby, room, Supabase, Netlify, Vite, character-selection, and responsive-game foundations while introducing the first complete v0.6 implementation pass.
...
Netlify uses `npm run build` and publishes `dist`. The existing working Supabase database can be reused without a schema migration. Start a new room when testing v0.6 because card names, canonical card IDs, and turn flags changed.

See `V0.6-IMPLEMENTATION-NOTES.md` and `V0.6-TEST-CHECKLIST.md` before any production deployment.
